    ...
    const loadUsers = async () => {
      try {
        ...
        const mappedUsers = response.data.map((user: any) => ({
          ...
          email: user.email,
          rol: user.role === 'cliente' ? 'Cliente' : user.role === 'admin' ? 'Administrador' : user.role,
          estado: user.status || 'activo',
          ultimoAcceso: user.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString() : 'No disponible',
          telefono: user.phone || '-',
          fechaCreacion: user.createdAt ? new Date(user.createdAt).toLocaleDateString() : '-',
        }));
